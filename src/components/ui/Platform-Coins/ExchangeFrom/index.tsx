...
const ExchangeFrom: React.FC<TProps> = ({ coins, selectedCoin, onSelect, callback }): JSX.Element => {
    ...
    const { balance, balances, balanceStatus, balancesStatus, rates, rateStatus } = useAppSelector(state => state.balances);
    ...
    React.useEffect(() => {
        if (selectedCoin && balances && rates && balance) {
    ...
    }, [selectedCoin, form, balances, balance, rates]);
